...
`pending/`에 문서를 저장·수정할 때 적용한다. `pending/`은 SoT에 반영하기 전 검토가 필요한 내용을 두는 로컬 작업 디렉터리이며, `.gitignore`로 git 추적에서 제외된다.
...
- `pending/`은 SoT가 아니다. 확정된 내용만 `resume-sot.md` 또는 `work-experience-sot.md`에 반영한다.
