...
`resume.html`과 `work-experience.html`을 생성·재생성하거나 스타일·레이아웃을 수정할 때 반드시 적용한다. 공통 템플릿 파일이나 빌드 도구 대신 이 규격으로 두 독립형 HTML의 디자인을 유지한다.

## 원천과 수정 범위

- 내용의 유일한 원천은 각각 `resume-sot.md`, `work-experience-sot.md`다. 이 문서는 디자인 규격이며 경력 내용의 원천이 아니다.
